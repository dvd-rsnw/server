...
def raw_feed_f() -> Dict[str, List[FeedEntity]]:
    ...
    target_stops = {'F24N', 'F24S'}  # 7 Av
    local_stops = {'F23N', 'F23S'}  # 4 Av-9th St
    all_valid_stops = target_stops | local_stops
    ...
    for entity in feed.entity:
        ...
        if trip_update.stop_time_update:
            # Filter stop time updates to only include our valid stops
            filtered_updates = []
            for stu in trip_update.stop_time_update:
                if stu.HasField('stop_id') and stu.stop_id in all_valid_stops:
                    filtered_updates.append(stu)
                    if stu.stop_id in target_stops and stu.HasField('arrival') and stu.arrival.HasField('time') and stu.arrival.time > one_minute_future:
                        has_future_target_stop = True
                    if stu.stop_id in local_stops:
                        is_local = True
            
            # If no valid stops or no future target stop, skip this entity
            if not filtered_updates or not has_future_target_stop:
                continue
                
            # Clear and repopulate stop time updates with our filtered list
            trip_update.ClearField('stop_time_update')
            trip_update.stop_time_update.extend(filtered_updates)
        ...
        feed_entity = FeedEntity(**feed_entity)
        if is_local:
            local_trains.append(feed_entity)
        else:
            express_trains.append(feed_entity)
    
    return {
        "express": express_trains,
        "local": local_trains
    } 